...
function calculateTotalInventory() {
  ...
  return items.reduce(
    (accumulator, curval) => accumulator + curval.inventory,
    0
  );
}
...
function calculateAllInventoryPrice() {
  ...
  return items.reduce((accumulator, curval) => accumulator + curval.price, 0);
}
...
function getItemPriceByName(name) {
  ...
  const filtered = items.find((item) => item.name === name);
